...
The \java{main} method will calculate the arithmetic, geometric, and harmonic means of 3.6, 7.0, and 11.0.
\end{exercise}
...
\begin{exercise}
Consider a rectangular prism (also known as a ``box''):

\begin{figure}[!ht]
\begin{center}
\includegraphics{figs/rectangular-prism.pdf}
...
Write a method named \java{cityBlockDistance(double x1, double y1, double x2, double y2)} that takes the coordinates of the two points and returns the city block distance between them. {\em Hint:} Use \java{Math.abs}.

The \java{main} method will prompt the user to enter the coordinates; it will then calculate the Pythagorean distance using the \java{distance} method from this chapter and print the returned result. Finally, \java{main} will call the \java{cityBlockDistance} method to calculate the city block distance and then print the returned result.
\end{exercise}
...
Write a method named \java{greatCircleDistance} which takes the latitude and longitude of two points {\em in degrees} and returns the great circle distance, which will be in kilometers.

You will also write a method named \java{kmToMiles} which takes as its single parameter a number of kilometer and returns the equivalent number of miles by multiplying by 0.621371.
