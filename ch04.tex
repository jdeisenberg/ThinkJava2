...
\label{sec:incrementaldevelopment}
...
\begin{exercise}
In Section~\ref{sec:incrementaldevelopment} we developed a formula for the {\em Pythagorean distance} between two points. Another way to measure the distance between two points is the ``city block`` distance: the sum of the absolute $x$ distance plus the absolute $y$ distance. For example, the city block distance from (1.5, 7) to (5, 2) is 8.5---the $x$ distance is 3.5 ($|1.5 - 5|$) and the $y$ distance is five ($|7 - 2|$).

Write a method named \java{cityBlockDistance} that takes the coordinates of the two points and returns the city block distance between them. {\em Hint:} Use \java{Math.abs} to calculate absolute value.
