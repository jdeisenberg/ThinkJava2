...
Hint: use \java{printf} to 

\begin{enumerate}
\item Start by defining variables for the item price (a \java{double}), number purchased (an \java{int}), the subtotal, amount of tax on the purchase, and the total.

\item Define a constant (\java {final}) for the sales tax and set it to 7.5\%.
%Instructions for downloading this code are on page~\pageref{code}.

\item Create a \java{Scanner} for doing input.

\item Compile and run this program.

\item Modify the program to prompt the user for the item price; then use the \java{Scanner} to read the price as a \java{double} and display the result. Compile and test the program.

\item Add the code to prompt the user for the quantity purchased; then use the \java{Scanner} to read the quantity as an \java{int} and display the result. Compile and test the program.
...
Consider a staircase as shown in the following figure:

\begin{figure}[!ht]
\begin{center}
\includegraphics{figs/staircase.pdf}
